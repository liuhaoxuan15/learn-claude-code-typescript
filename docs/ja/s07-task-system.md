...
class TaskManager {
    ...
    create(subject: string, description = ""): string {
        const task = {"id": this._nextId, subject,
...
```typescript
    private clearDependency(completedId: number): void {
        for (const f of fs.readdirSync(this.dir)) {
            ...
            const blockedBy = task["blockedBy"] as number[];
            if (blockedBy.includes(completedId)) {
                blockedBy.splice(blockedBy.indexOf(completedId), 1);
                this.save(task);
            }
        }
    }
...
```typescript
    update(taskId: number, status?: string,
           addBlockedBy?: number, addBlocks?: number): void {
...
const TOOL_HANDLERS = {
    // ...base tools...
    "task_create": (p: any) => TASKS.create(p["subject"]),
    "task_update": (p: any) => TASKS.update(p["task_id"], p.get("status")),
    "task_list":   () => TASKS.listAll(),
    "task_get":    (p: any) => TASKS.get(p["task_id"]),
};
```
