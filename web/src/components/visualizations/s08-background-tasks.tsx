...
const STEP_INFO: StepInfo[] = [
  {
    title: "三条通道",
    description:
      "Agent 有一个主线程，可以派生子守护线程进行并行工作。",
  },
  {
    title: "主线程工作中",
    description:
      "主 agent 循环照常运行，处理用户请求。",
  },
  {
    title: "派生后台任务",
    description:
      "后台任务作为守护线程运行。主循环不等待它们。",
  },
  {
    title: "多个后台任务",
  ...
  },
  {
    title: "队列填充",
    description:
      "结果在队列中累积，在这一轮中对模型不可见。",
  },
  {
    title: "清空队列",
    description:
      "就在下一次 LLM 调用之前，所有排队的通知作为 tool_results 注入。非阻塞，异步。",
  },
];
        ...
        <svg viewBox="0 0 780 380" className="w-full" aria-label="Background task lanes">
          ...
          {(
            [
              ...
              { key: "bg1", y: LANE_Y.bg1, label: "后台任务 1" },
              { key: "bg2", y: LANE_Y.bg2, label: "后台任务 2" },
            ] as const
          ).map(({ key, y, label }) => (
            <g key={key}>
              <rect
